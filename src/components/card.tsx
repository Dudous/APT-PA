...
export const Card: React.FC<{image: string, title: string, text: string, time: string, read: number, views: number, comments: number, bgcolor: string, txtcolor: string}> =
    ({image, title, text, time, read, views, comments, bgcolor, txtcolor}) => {
    return (
        <div className="flex flex-col w-96 h-[60vh] justify-between bg-white">
            ...
            <div className="flex flex-col h-4/5 px-4 items-center">
                <div className="flex flex-col justify-center text-center py-4">
                    <p className={`${txtcolor}`}>{time}</p>
                    <h2 className="text-3xl font-bold">{title}</h2>
                </div>
                <p className="text-center my-6 text-gray-400">{text}</p>
            </div>
            <div className={`flex flex-row w-full h-1/5 justify-between divide-x divide-gray-400 text-white ${bgcolor}`}>
